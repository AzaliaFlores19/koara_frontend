  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      <button
        ...
        className="w-full flex items-center justify-between gap-3 bg-white border-2 border-slate-900 rounded-full pl-6 pr-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-all active:scale-[0.98]"
      >
        <span>{value}</span>
        <ChevronDown
          className={`text-slate-400 transition-transform duration-300 ${isOpen ? "rotate-180" : ""} -ml-2`}
          size={16}
        />
      </button>
